...
# send text to the AI and pull out the facts
def extract(text):
...
If a field is not found, write N/A."""},  # instructions to the ai
            {"role": "user", "content": f"Text: {text}"}  # the sentence we send
        ]
    )
    return response.choices[0].message.content.strip()  # first response only
...
for i in range(len(samples)):  # loop over every text
    text = samples[i]
